...
@media (prefers-reduced-motion: reduce) {
  .compare-bar { transition: none; transform: scaleX(var(--bar-ratio, 1)); }
}
...
(function() {
  ...
  // Growth-loop cadence: ~2s grow, ~3.5s hold, quick dissolve, repeat
  const TICK_MS = 70;        // ms between growth steps
  const GROW_RATE = 1.5;     // average front advance per tick (in cells) — radial wave
  ...
  function step() {
    ...
    if (cycle < growTicks) {
      // growing: the front swells with an ease-in-out wave — slow burst from
      // the heart, full speed through the body, gentle settle at the fins
    ...
    for (let i = 0; i < whaleOrder.length; i++) {
      ...
      if (target === 1) {
        if (!cells[key]) cells[key] = { a: 0.12, ta: pickAlpha(), phase: Math.random() * 6.28 };
...
(function() {
  ...
  COMPARE.forEach(function(b) {
    ...
    const maxV = sorted.reduce(function(acc, m) { return (m.v != null && m.v > acc) ? m.v : acc; }, 0);
    sorted.forEach(function(m) {
      ...
      bar.style.setProperty('--bar-ratio', m.v == null ? '0' : Math.max(0.02, m.v / maxV).toFixed(4));
      track.appendChild(bar);
      const score = document.createElement('span');
